...
from src.services.bigcommerce.services.stored_instruments_fetch import (
    get_stored_instruments as bc_get_stored_instruments,
)
from src.services.bigcommerce.services.address_fetch import (
    get_by_customer_id as bc_get_addresses_by_id,
)
...
def transform_to_braintree_customer(bigcommerce_customer_id):
    ...
    braintree_customer_request = {
        ...
        "payment_methods": __get_payment_methods_methods(
            bc_customer_stored_instruments
        ),
...
def __get_addresses(bc_customer_addresses):
    ...
    def process_address(add):
        (
            ...
            short_name=add.get("country", None),
            code_alpha2=add.get("country_code", None),
        )
        return {
...
def __get_credit_cards_methods(bc_customer_stored_instruments):
    ...
    """
    from src.services.paypal.braintree.const.payment_method_types import CARD_BRANDS
    ...
    for si in bc_customer_stored_instruments:
        ...
        card_type = CARD_BRANDS.get(si.get("brand", "").upper(), "")
